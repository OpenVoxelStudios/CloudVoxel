...
function isBot(userAgent: string | null) {
    ...
    const botPatterns = [
        'bot', 'crawler', 'spider', 'discord', 'whatsapp', 'telegram',
...
export const GET = auth(async (req: NextRequest, { params }: { params: Promise<{ path?: string[] }> }): Promise<NextResponse> => {
    ...
    const root = typeof ROOT === 'string' ? ROOT : (find ? ROOT[find].path : undefined);
    ...
    const response = new NextResponse(readStream as unknown as ReadableStream, {
        status: 200,
        headers: {
            'Content-Type': (req.nextUrl.searchParams.get('download') == 'true' ? false : mimeType) || 'application/octet-stream',
