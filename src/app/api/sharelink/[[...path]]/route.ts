...
export const GET = auth(
  async (
    ...
    if (req.headers.get("Authorization")) {
      ...
      if (!hasValidToken)
        return NextResponse.json(
          { error: "Unauthorized API key for permission files:share." },
          { status: 401 },
        );
    }
    ...
    logger.log(
      `<${req.auth?.user?.email || `API-${req.headers.get("Authorization")}`}> Generating share code (first time) for "${fileName}" file at ${filePath}`,
    );
