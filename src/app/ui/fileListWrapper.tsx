...
export default function FileListWrapper({
  ...
  pathParts: string[];
  initialFiles: FileElement[] | FetchError | null;
}) {
  ...
  const [sortOrder, setSortOrder] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("preferences.sortOrder")
        ? localStorage.getItem("preferences.sortOrder") === "true"
        : true;
    }
    return true;
  });
  const [sortBy, setSortBy] = useState<(typeof sortOptions)[number]>(() => {
    if (typeof window !== "undefined") {
      ...
      return stored &&
        sortOptions.includes(stored as (typeof sortOptions)[number])
        ? (stored as (typeof sortOptions)[number])
        : "name";
    }
    return "name";
  });
  ...
  const debouncedSetSortBy = useMemo(
    () =>
      debounce((value: (typeof sortOptions)[number]) => {
        if (typeof window !== "undefined") {
  ...
  const debouncedSetSortOrder = useMemo(
    () =>
  ...
  const debouncedSetPartition = useMemo(
    () =>
