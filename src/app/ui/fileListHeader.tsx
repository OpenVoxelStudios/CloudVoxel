...
export default function FileListHeader({
  ...
  const handleAddDirectory = async (name: string) => {
    const res = await fetch(
      `/api/dashboard/${pathParts.map(encodeURIComponent).join("/")}/${encodeURIComponent(name)}?folder=true`,
      {
        method: "POST",
        cache: "no-cache",
        headers: {
    ...
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mb-4">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          >
            <div
              className={`transform transition-transform duration-200${sortOrder ? " rotate-180" : ""}`}
            >
