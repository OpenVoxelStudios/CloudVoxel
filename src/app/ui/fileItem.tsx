...
const MemoizedFileItem = React.memo(function FileItem({
                ...
                url:
                  hash && code
                    ...
                    : new Promise(async (resolve) => {
                        const res = await fetch(
                          `/api/sharelink/${pathParts.map(encodeURIComponent).join("/")}/${encodeURIComponent(name)}`,
                          {
                            headers: {
                              Partition: partition || "",
                            },
                        ...
                        if (res.status == 200) {
                          resolve(
                            `${location.origin}/api/share/${pathParts.concat(name).map(encodeURIComponent).join("/")}?hash=${json.hash}&code=${json.code}${partition ? `&partition=${encodeURIComponent(partition)}` : ""}`,
                          );
                        } else {
                          toast({
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger>
        <div
          className="flex items-center justify-between p-4 bg-gray-800 border-b border-gray-700 hover:bg-gray-750 transition-colors duration-150 ease-in-out"
          data-test="fileitem"
        >
          <Link
