...
const ProviderConfig = {
  redirectProxyUrl: `${clientconfig.websiteURL}/api/auth`,
...
export const providers: Provider[] = (
      ...
      Credentials({
        ...
        authorize: async (credentials, req) => {
          ...
          const user = await db
            .select()
            .from(usersTable)
            .where(eqLow(usersTable.email, email))
            .get();
  ...
  .filter(
    (provider) =>
      (provider[0] == "passkey" && config.enableExperimentalPasskeys) ||
      (provider[0] == "credentials" && config.credentialLogin) ||
      (
        config.providers.map((p) => p.toLowerCase()) as unknown as string[]
      ).includes(provider[0].toLowerCase()),
  )
