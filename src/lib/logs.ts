...
if (config.logs.folder && !existsSync(config.logs.folder)) mkdirSync(config.logs.folder, { recursive: true });
...
export function error(error: string | Error): void {
    writeLog(error instanceof Error ? `${error.name}: ${error.message}\nError trace: ${error.stack}` : error, 'error');
}
